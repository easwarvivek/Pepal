	...
	return helper.getRegisteredUsers(username, org).then((user) => {
		...
		if (all_good) {
			logger.debug(util.format(
				...
				.signature));
			log_info = proposalResponses[0].response
			var request = {
		...
		if (response.status === 'SUCCESS') {
			...
			//log_info = {};
			//log_info["message"] = "Failed to order the transaction";
			//log_info["status"] = 500;
			return log_info
		}
	}, (err) => {
		...
		log_info["status"] = 500;
		log_info["message"] = 'Failed to send transaction due to error: ' + err.stack ? err.stack : err;
		log_info["payload"] = {"type":"Buffer", "data":[]};
		return log_info
	});
};
